...
* 激活函数: h(x)  
    1. 阶跃函数
	    1. 图示  
	    ![跃迁函数图示](https://github.com/ZWtu19/Notes/blob/master/src/pics/阶跃函数.png)
	    2. 函数表达式为： 略
    ...
    |函数|相同处|不同处|  
    |:-------|:-------|  
    |阶跃函数&sigmoid函数|宏观上来看:形状相似;种类来看:均为非线性函数;| 平滑性不同:阶跃函数输出0/1,sigmoid函数可以输出连续的值|  
	
    4. ReLu函数：  
    	1. 图示：  
    	![sigmoid函数](https://github.com/ZWtu19/Notes/blob/master/src/pics/ReLU函数.png)
	    2. 函数的表达式为:  
	    ![sigmoid函数](https://github.com/ZWtu19/Notes/blob/master/src/pics/ReLU函数表达式.png)
	    3. 在Python中的实现方式  
	    ```python
        # 其中np为Numpy, x 可以为Numpy数组
            def relu(x):
...
* 神经网络的信号传递实现三层为例  
![权重符号说明](https://github.com/ZWtu19/Notes/blob/master/src/pics/三层神经网络.png)  
    1. 符号说明  
...
* mini-batch的实现  
    详见代码和结果图;步骤如下:  
    1. 设置mini-batch的大小为100,需要每次从60000个训练数据中随机取出100个数据;
    2. 对包含100笔数据的mini-batch求梯度,使用随机梯度下降法更新参数;  
    3. 最终结果可见图像展示;  
    结果: 观察以上结果,可以发现随着学习的进行,损失函数的值在不断减小,这意味着神经网络的权重参数在逐渐拟合数据,换言之,神经网络趋势在学习并组建向最优参数靠近;  
    
* 基于测试数据的评价 
* 学习总结:  
    1. 机器学习中使用的数据集分别为训练数据和测试数据;  
    2. 神经网络用训练数据进行学习,并用测试数据评价学习到的模型的泛化能力;  
    3. 神经网络的学习以损失函数为指标,更新权重参数,使损失函数的值减少;  
    4. 利用某个给定的微小值的差分求导数的过程,称为数值微分;  
    5. 利用数值微分,求得计算权重参数的梯度;  
    
#### 4.误差反向传播学习法  
* 理解  
    1. 在通过神经网络的学习过程中,需要不断求得损失函数关于权重参数的梯度,用以求得最优的权重参数;  
    2. 在以上的学习中,通过数值微分的方法计算梯度,存在计算时间长的缺点;  
    3. 为解决上述问题,在计算梯度时使用误差反向传播法高效地计算权重;  
    4. 基于数学式理解: 略;  
    5. 基于计算图理解: 见下文;  
...
* 计算图  
    1. 用计算图求解 
        1. 步骤一: 构建计算图;  
        2. 步骤二: 在计算图上,从左向右进行计算;  
        见下图:   
        
    2. 用局部计算  
        计算图可以集中精力用于局部计算.无论全局计算有多么复杂,各个步骤所要做的就是对象节点的局部计算,并通过计算结果获得全局的复杂计算的结果;  
        见下图,只需计算苹果的价格,再与4000元相加即可,至于4000元如何通过复杂计算求得无须关注:  
    
    3. 用计算图解题的原因  
        1. 优点一: 局部计算;  
        2. 优点二: 利用计算图可以将中间的计算结果全部保存起来;
        3. 最重要原因: 可以额通过反向传播高效计算导数;  
    
* 链式法则  
    1. 计算图的反向传播  
    传播过程见下图: ![链式法则](https://github.com/ZWtu19/Notes/blob/master/src/pics/链式法则.png)  
    计算顺序: 将信号E乘以节点的局部导数,然后将结果传递给下一个节点.  
    
    2. 反向传播的实现: 链式法则  
        
    3. 链式法则和计算图  
    计算过程见下图:  ![链式法则和计算图](https://github.com/ZWtu19/Notes/blob/master/src/pics/链式法则和计算图.png)
    
* 反向传播  
    1. 加法节点的反向传播(以z = x + y为例):   
        ![加法节点的反向传播](https://github.com/ZWtu19/Notes/blob/master/src/pics/加法节点的反向传播.png)  
        
    2. 乘法节点的反向传播(以z = x * y 为例):  
        ![乘法节点的反向传播](https://github.com/ZWtu19/Notes/blob/master/src/pics/乘法节点的反向传播.png)
    3. 举例计算(以购买苹果和句子的反向传播为例)  
        计算图如下:   
        ![购买苹果和香蕉的计算图](https://github.com/ZWtu19/Notes/blob/master/src/pics/苹果和句子的反向传播计算图.png)
        代码实现:详见Projects/1.神经网络/Codes/4.误差反向传播算法/Mulayer.py  
          
* 简单层的实现  
    1. 乘法层的实现:详见Projects/1.神经网络/Codes/4.误差反向传播算法/Mulayer.py   
    2. 加法层的实现:详见Projects/1.神经网络/Codes/4.误差反向传播算法/addlayer.py   
    
* 激活函数层的实现  
    1. Relu层  
        1. 计算图如下:  
        ![激活函数层-Relu](https://github.com/ZWtu19/Notes/blob/master/src/pics/激活函数-Relu层.png)  
        2. 代码实现  
        ```python
       class Relu:
           def __init__(self):
           ...
           def backward(self, dout): 
               ...
               return dx
        ```  
       3. 理解Relu激活函数:可以形象地将Relu层理解为电路中的开关;    
       
    2. Sigmoid层  
        1. 计算图如下:  
        ![激活函数层-Sigmoid](https://github.com/ZWtu19/Notes/blob/master/src/pics/激活函数-Sigmoid层.png)
        2. 代码实现  
        ```python
       class Sigmoid:
           def __init__(self):
...
* Affine/Softmax层的实现  
       1. Affile层  
            1. 在使用神经网络时,我们在隐层需要计算加权信息;转换为计算图如下:  
            ![Affile计算图](https://github.com/ZWtu19/Notes/blob/master/src/pics/affile计算图.png)  
            2. 求偏导数结果如下:  
            ![Affile偏导数](https://github.com/ZWtu19/Notes/blob/master/src/pics/affile偏导数.png)  
           
        2. Softnmax层: 略
        
* 误差反向传播算法的实现  
    1. 神经网络全貌图  
        * 前提:神经网络中有合适的权重和偏置，调整权重和偏置以便拟合训练数据的 过程称为学习  
        1. 步骤一:  从训练数据中随机选择一部分数据;
        2. 步骤二:  计算损失函数关于各个权重参数的梯度;
        3. 步骤三:  将权重参数沿梯度方向进行微小的更新;
        4. 步骤四:  重复步骤 1、步骤 2、步骤 3;  
     
    2. 对应误差反向传播法的神经网络的实现  
        将构建TwoLayerNet类,其中实例变量和方法整理如下:  
        ...
        |方法|说明|
        |:--------|:--------|
        |__init__|进行初始化。参数从头开始依次是输入层的神经元数、隐藏层的神经元数、输出层的神经元数、初始化权重时的高斯分布的规模|
        |predict(self, x)|进行识别(推理)。 参数 x 是图像数据|  
...
* 总结  
    1. 使用计算图直观地把握计算过程  
    2. 计算图的节点是有局部计算构成的. 局部计算构成全局计算  
    3. 计算图的正向传播进行一般的计算.通过计算图的反向传播可以计算各个节点的导数  
    4. 通过将神经网络的组成元素实现为层,可以高效地计算梯度(反向传播法)  
    5. 通过比较数值微分和误差反向传播法的结果,可以确认误差反向传播法的实现是否正确  
